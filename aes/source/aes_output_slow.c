...
/*-------------------------------------------------------------------------- */
/*
Процедура печати матрицы состояния
*/
DEBUG_ONLY(
void print_state(struct aes_context *ctx, const char *prefix, int round) {    
...
/*-------------------------------------------------------------------------- */
/*
Процедура печати ключа шифрования
*/
void print_key(struct aes_context *ctx, const char *prefix, int round) {
...
/*-------------------------------------------------------------------------- */
void inv_print_key(struct aes_context *ctx, const char *prefix, int round) {
	...
	sp = (uint8_t *)(ctx->w + AES_NB * (ctx->nr - round));
...
void aes_encrypt(struct aes_context *ctx, uint8_t *input, uint8_t *output) { 
	...
	DEBUG_ONLY(print_state(ctx, "input ", 0));
 	...
 	add_round_key(ctx, 0);
	DEBUG_ONLY(print_key(ctx, "k_sch ", 0));
	...
	for (int i = 1; i < ctx->nr; i++) {
		...
		add_round_key(ctx, i);
		DEBUG_ONLY(print_key(ctx, "k_sch ", i));
	}	
	DEBUG_ONLY(print_state(ctx, "istart", ctx->nr));

	sub_bytes(ctx->state);
	DEBUG_ONLY(print_state(ctx, "s_box ", ctx->nr));

	shift_rows(ctx->state);
	DEBUG_ONLY(print_state(ctx, "s_row ", ctx->nr));

	add_round_key(ctx, ctx->nr);
	DEBUG_ONLY(print_key(ctx, "k_sch ", ctx->nr));
	...
	DEBUG_ONLY(print_state(ctx, "output", ctx->nr));
}
...
void aes_decrypt(struct aes_context *ctx, uint8_t *input, uint8_t *output) { 
	...
	add_round_key(ctx,  ctx->nr);
	DEBUG_ONLY(print_key(ctx, "ik_sch", 0));

	for (int i = ctx->nr - 1; i > 0; i-- ) {
		print_state(ctx, "istart", ctx->nr - i);

		inv_shift_rows(ctx->state);
		DEBUG_ONLY(print_state(ctx, "is_row", ctx->nr - i));

		inv_sub_bytes(ctx->state);
		DEBUG_ONLY(print_state(ctx, "is_box",  ctx->nr - i));

		add_round_key(ctx, i);
		DEBUG_ONLY(print_key(ctx, "ik_sch", ctx->nr - i));

		DEBUG_ONLY(print_state(ctx, "ik_add",  ctx->nr - i));
		inv_mix_columns(ctx->state);
	}	
	DEBUG_ONLY(print_state(ctx, "istart", ctx->nr));

	inv_shift_rows(ctx->state);
	DEBUG_ONLY(print_state(ctx, "is_row", ctx->nr));

	inv_sub_bytes(ctx->state);	
	DEBUG_ONLY(print_state(ctx, "is_box",  ctx->nr));

	add_round_key(ctx, 0);
	DEBUG_ONLY(print_key(ctx, "ik_sch", ctx->nr));

	DEBUG_ONLY(printf("round[%2d].output  ",ctx->nr));
	for (int i = 0; i < AES_NB; i++) {
			for (int j = 0; j < AES_NB; j++) {
				output[i * AES_NB + j] = sp[j * AES_NB + i];
				DEBUG_ONLY(printf("%02x", output[i * AES_NB + j]));
			}
		}
	DEBUG_ONLY(printf("\n"));
}
