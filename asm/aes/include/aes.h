...
/* Процедура шифрует 128-битный блок 'in' и подставляет его в 128-битный блок 'out'. */
void aes_encrypt(struct aes_context *ctx, const uint8_t *in, uint8_t *out);
/* Процедура дешифрует 128-битный блок 'in' и подставляет его в 128-битный блок 'out'.*/
void aes_decrypt(struct aes_context *ctx, const uint8_t *in, uint8_t *out);
